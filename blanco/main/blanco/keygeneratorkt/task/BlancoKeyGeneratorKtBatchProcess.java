...
/**
 * Batch process class [BlancoKeyGeneratorKtBatchProcess].
 *
 * <P>Example of a batch processing call.</P>
 * <code>
 * java -classpath (classpath) blanco.keygeneratorkt.task.BlancoKeyGeneratorKtBatchProcess -help
 * </code>
 */
public class BlancoKeyGeneratorKtBatchProcess {
    /**
     * Normal end.
     */
    ...
    public static final void main(final String[] args) {
        final BlancoKeyGeneratorKtBatchProcess batchProcess = new BlancoKeyGeneratorKtBatchProcess();

        // Arguments for batch process.
        final BlancoKeyGeneratorKtProcessInput input = new BlancoKeyGeneratorKtProcessInput();
        ...
        for (int index = 0; index < args.length; index++) {
            ...
            if (arg.startsWith("-verbose=")) {
                ...
                System.out.println("BlancoKeyGeneratorKtBatchProcess: The input parameter[" + arg + "] was ignored.");
                isNeedUsage = true;
            }
        }
        ...
        if( isFieldMetadirProcessed == false) {
            System.out.println("BlancoKeyGeneratorKtBatchProcess: Failed to start the process. The required field value[metadir] in the input parameter[input] is not set to a value.");
     ...
     */
    public int process(final BlancoKeyGeneratorKtProcessInput input) throws IOException, IllegalArgumentException {
        // Checks the input parameters.
        validateInput(input);

        // If you get a compile error at this point, You may be able to solve it by implementing a BlancoKeyGeneratorKtProcess interface and creating an BlancoKeyGeneratorKtProcessImpl class in package blanco.keygeneratorkt.task.
        final BlancoKeyGeneratorKtProcess process = new BlancoKeyGeneratorKtProcessImpl();
     ...
     * @throws IllegalArgumentException If an invalid input value is found.
     */
    public final int execute(final BlancoKeyGeneratorKtProcessInput input) throws IllegalArgumentException {
        try {
            ...
            System.out.println("BlancoKeyGeneratorKtBatchProcess: An input exception has occurred. Abort the batch process.:" + ex.toString());
            // Termination due to abnormal input.
            return END_ILLEGAL_ARGUMENT_EXCEPTION;
        } catch (IOException ex) {
            System.out.println("BlancoKeyGeneratorKtBatchProcess: An I/O exception has occurred. Abort the batch process.:" + ex.toString());
            // Termination due to abnormal input.
            return END_IO_EXCEPTION;
        } catch (RuntimeException ex) {
            System.out.println("BlancoKeyGeneratorKtBatchProcess: A runtime exception has occurred. Abort the batch process.:" + ex.toString());
            ex.printStackTrace();
            // Abnormal end.
            return END_ERROR;
        } catch (Error er) {
            System.out.println("BlancoKeyGeneratorKtBatchProcess: A runtime exception has occurred. Abort the batch process.:" + er.toString());
            er.printStackTrace();
            // Abnormal end.
            return END_ERROR;
        }
    ...
    public static final void usage() {
        System.out.println("BlancoKeyGeneratorKtBatchProcess: Usage:");
        System.out.println("  java blanco.keygeneratorkt.task.BlancoKeyGeneratorKtBatchProcess -verbose=value1 -metadir=value2 -targetdir=value3 -tmpdir=value4 -encoding=value5 -xmlrootelement=value6 -sheetType=value7 -targetStyle=value8 -lineSeparator=value9 -packageSuffix=value10 -overridePackage=value11 -searchTmpdir=value12");
     ...
     * @throws IllegalArgumentException If an invalid input value is found.
     */
    public void validateInput(final BlancoKeyGeneratorKtProcessInput input) throws IllegalArgumentException {
        if (input == null) {
        ...
        if (input.getMetadir() == null) {
            throw new IllegalArgumentException("BlancoKeyGeneratorKtBatchProcess: Failed to start the process. The required field value[metadir] in the input parameter[input] is not set to a value.");
        }
    }
}
